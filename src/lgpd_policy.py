...
def list_policy_versions(
    domain: str, policies_root: Path = POLICIES_ROOT
) -> list[Path]:
    ...
    versioned = [
        path
        for path in domain_dir.glob("v*.json")
        if _extract_version(path) is not None
    ]
    if not versioned:
        raise FileNotFoundError(
            f"Nenhuma política LGPD versionada encontrada em: {domain_dir}"
        )
    return sorted(versioned, key=lambda path: _extract_version(path) or 0)


def load_policy(
    domain: str, version: int | None = None, policies_root: Path = POLICIES_ROOT
) -> dict[str, Any]:
    ...
    if version is None:
        ...
        if selected not in versions:
            raise FileNotFoundError(
                f"Versão da política LGPD não encontrada: {selected}"
            )
