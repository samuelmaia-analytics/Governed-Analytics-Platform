...
def render_report(logistics: pd.DataFrame, seller: pd.DataFrame, cohort: pd.DataFrame) -> str:
    lines = [
...
def run_semantic_layer() -> SemanticArtifacts:
    ...
    REPORT_PATH.write_text(render_report(logistics, seller, cohort), encoding="utf-8")
