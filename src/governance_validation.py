...
DEFAULT_GOVERNANCE_CONTRACT_PATH = ROOT_DIR / "contracts" / "governance" / "release_governance.json"
...
def validate_governance_contract(contract: dict[str, Any]) -> list[GovernanceValidationResult]:
    ...
    branch_by_environment = {
        branch["deployment_environment"]: branch for branch in branches if isinstance(branch, dict)
    }
    environment_by_target = {
        environment["target_environment"]: environment for environment in environments if isinstance(environment, dict)
    }

    for environment_name, profile in DEPLOYMENT_PROFILES.items():
        branch = branch_by_environment.get(environment_name)
        if branch is None:
            results.append(GovernanceValidationResult("FAIL", f"Branch policy ausente para `{environment_name}`."))
            continue
        ...
        if environment is None:
            results.append(GovernanceValidationResult("FAIL", f"GitHub environment ausente para `{environment_name}`."))
            continue
    ...
    if not results:
        results.append(GovernanceValidationResult("PASS", "Contrato de governança alinhado aos perfis de release."))
    return results
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Valida o contrato versionado de governança de release.")
