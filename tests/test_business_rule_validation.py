...
def test_rule_passes_when_failure_rate_is_within_contractual_tolerance() -> None:
    df = pd.DataFrame(
        {
    ...
    contract = _valid_contract(
        [
            {
                "rule_id": "delivered_requires_delivery_date",
                "severity": "high",
                "type": "not_null_if",
                "column": "order_delivered_customer_date",
                "params": {
    ...
    assert "tolerancia contratual" in results[0].details
