    ...
    return pd.DataFrame(
        {
            ...
            "order_delivered_customer_date": pd.to_datetime(
                ["2018-01-05", "2018-01-05", "2018-01-08"]
            ),
            "order_estimated_delivery_date": pd.to_datetime(
                ["2018-01-04", "2018-01-04", "2018-01-07"]
            ),
...
def test_dimension_join_coverage_fails_when_missing_pct_exceeds_threshold() -> None:
    ...
    results = {
        result.check_name: result for result in check_dimension_join_coverage(df)
    }

    assert results["dimension_join_missing_pct__customer_state"].status == "FAIL"
    assert (
        float(results["dimension_join_missing_pct__customer_state"].metric_value) > 1.0
    )
