...
def test_build_metrics_and_dataset_scorecards(tmp_path: Path, monkeypatch) -> None:
    ...
    monitoring_path = (
        tmp_path / "published" / "monitoring" / "published_layer_monitoring.csv"
    )

    _write_csv(
        quality_path,
        [
            {"check_name": "q1", "status": "PASS", "severity": "high"},
            {"check_name": "q2", "status": "FAIL", "severity": "medium"},
        ],
    )
    _write_csv(
        business_rules_path,
        [
            {"rule_id": "r1", "status": "PASS", "severity": "high"},
            {"rule_id": "r2", "status": "PASS", "severity": "low"},
        ],
    )
    _write_csv(
        schema_path,
    ...
    _write_csv(
        privacy_path,
        [
            {"check_name": "p1", "status": "PASS"},
            {"check_name": "p2", "status": "PASS"},
        ],
    )
    _write_csv(
        monitoring_path, [{"check_name": "m1", "status": "PASS", "severity": "high"}]
    )
    ...
    monkeypatch.setattr(
        scorecards, "PUBLISHED_MONITORING_RESULTS_PATH", monitoring_path
    )
...
def test_save_outputs_and_report(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(
        scorecards, "PUBLISHED_MONITORING_DIR", tmp_path / "published" / "monitoring"
    )
    monkeypatch.setattr(scorecards, "DOCS_DIR", tmp_path / "docs")
    monkeypatch.setattr(
        scorecards,
        "SCORECARD_CSV_PATH",
        tmp_path / "published" / "monitoring" / "governance_scorecards.csv",
    )
    monkeypatch.setattr(
        scorecards,
        "SCORECARD_JSON_PATH",
        tmp_path / "published" / "monitoring" / "governance_scorecards.json",
    )
    monkeypatch.setattr(
        scorecards, "REPORT_PATH", tmp_path / "docs" / "governance_scorecards.md"
    )
    metrics = [
        scorecards.ScorecardMetric(
            "fact_orders_dashboard", "published_monitoring", 100.0, "healthy", "x", 5, 0
        )
    ]
    dataset_cards = [
        scorecards.DatasetScorecard("fact_orders_dashboard", 100.0, "healthy")
    ]
