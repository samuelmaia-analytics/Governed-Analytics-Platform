...
def test_run_catalog_publication_returns_summary(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        "load_manifest",
        lambda path: [  # type: ignore[arg-type]
            CatalogAssetSpec(
                display_name="App",
    ...
    monkeypatch.setattr(
        platform_publication, "build_catalog_client", lambda base_url: DummyClient()
    )
    monkeypatch.setattr(
        platform_publication,
        "sync_assets",
        lambda client, assets, dry_run: [
            SyncResult("would_create", "App", None, "https://example.com/app")
        ],
    )
...
def test_run_pipeline_publication_supports_existing_pipeline_dry_run(
    ...
    monkeypatch.setattr(
        platform_publication, "build_pipeline_client", lambda base_url: DummyClient()
    )
    monkeypatch.setattr(
        platform_publication,
...
def test_to_project_path_returns_relative_path_inside_repo(tmp_path: Path) -> None:
    ...
    assert (
        platform_publication.to_project_path(relative_path)
        == "docs/platform_publication.md"
    )
...
def test_run_pipeline_publication_requires_name_field(tmp_path: Path) -> None:
    ...
    definition_path.write_text(
        json.dumps({"display_name": "missing-name"}), encoding="utf-8"
    )
...
def test_run_pipeline_publication_creates_and_executes_pipeline(
    monkeypatch, tmp_path: Path
) -> None:
    definition_path = tmp_path / "pipeline.json"
    definition_path.write_text(json.dumps({"name": "olist-pipeline"}), encoding="utf-8")
    ...
    class DummyClient:
        ...
        def run_pipeline(
            self, pipeline_id: str, payload: dict[str, str]
        ) -> dict[str, str]:
    ...
    client = DummyClient()
    monkeypatch.setattr(
        platform_publication, "build_pipeline_client", lambda base_url: client
    )
    monkeypatch.setattr(
        platform_publication,
        "find_pipeline_by_name",
        lambda client, pipeline_name: None,
    )
...
def test_build_clients_validate_settings_and_forward_credentials(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        platform_publication, "load_app_settings", lambda: DummySettings()
    )
    monkeypatch.setattr(
        platform_publication,
        "DadosferaMaestroClient",
        lambda **kwargs: ("catalog", kwargs),
    )
    monkeypatch.setattr(
        platform_publication,
        "DadosferaPipelineClient",
        lambda **kwargs: ("pipeline", kwargs),
    )

    catalog_client = platform_publication.build_catalog_client(
        "https://maestro.example.com"
    )
    pipeline_client = platform_publication.build_pipeline_client(
        "https://maestro.example.com"
    )
...
def test_main_generates_report_with_selected_steps(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: platform_publication.PlatformPublicationResult(
            "pipeline_publication", "DRY_RUN", "ok"
        ),
    )
