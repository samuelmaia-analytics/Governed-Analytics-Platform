...
def test_build_lineage_edges_returns_expected_core_flow() -> None:
    ...
    assert any(edge.output == "data/published/dashboard/fact_orders_dashboard.parquet" for edge in edges)
...
def test_save_lineage_outputs(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(lineage, "LINEAGE_JSON_PATH", tmp_path / "catalog" / "technical_lineage.json")
    monkeypatch.setattr(lineage, "LINEAGE_REPORT_PATH", tmp_path / "docs" / "technical_lineage.md")
    edges = lineage.build_lineage_edges()
