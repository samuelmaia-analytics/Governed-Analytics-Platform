...
def test_run_semantic_layer_materializes_all_slices(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(semantic_layer, "EXECUTIVE_KPI_PATH", semantic_dir / "executive_kpis_slice.parquet")
    ...
    assert artifacts.executive_kpi_path.exists()
    assert (semantic_dir / "logistics_slice.csv").exists()
    assert (semantic_dir / "category_slice.csv").exists()
    assert (semantic_dir / "executive_kpis_slice.csv").exists()
