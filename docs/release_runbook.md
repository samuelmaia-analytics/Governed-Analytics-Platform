...
3. Se a mudança impactar dados publicados, regenerar os artefatos críticos:

```bash
python src/run_case_pipeline.py --steps build publish quality contracts catalog queries bi
```

4. Revisar se os arquivos abaixo refletem a mudança:
...
2. Confirmar execução dos workflows de CI e deploy.
3. Validar o app Streamlit e os principais links públicos do case.
